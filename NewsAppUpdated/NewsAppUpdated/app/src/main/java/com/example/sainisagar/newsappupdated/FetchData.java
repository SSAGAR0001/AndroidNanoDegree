...
{
    public static final String LOG = FetchData.class.getSimpleName();
    public static final String NO = "No Author Avaiable";
    public static final String REF_ID = "webTitle";
    ...
    private static ArrayList<News> extractNews(String JSONnews) {
        ...
        try {
            ...
            for (int i = 0; i < result.length(); i++) {
                ...
                category = jsonObject.getString("sectionName");
                JSONArray jTags = (JSONArray) jsonObject.get("tags");
                String author = NO;
                if(jTags.length() != 0){
                    JSONObject authorJSON = (JSONObject)jTags.get(0);
                    author = authorJSON.getString(REF_ID).split("/")[0];
                }
                link = jsonObject.getString("webUrl");
                try {
                    publishedOn = formatter.parse(jsonObject.getString("webPublicationDate"));
                } catch (java.text.ParseException e) {
                    publishedOn = null;
                }
                news.add(new News(title, description, category, author, publishedOn, link));
            }
            return news;
